...
public final class LHU extends Instruction {
  ...
  public void execute(MachineCode code, State state) throws SimulationException {
    ...
    state.xregfile().setRegister(code.get(InstructionField.RD), state.memory().loadHalfUnsigned(addr));
  ...
  public String disassemble(MachineCode code) {
    ...
    return String.format("lhu x%d, x%d, %d", rd, rs1, imm);
  }
