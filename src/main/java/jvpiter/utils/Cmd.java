...
import jvpiter.Jvpiter;
import jvpiter.Logger;
import jvpiter.riscv.hardware.Cache.ReplacePolicy;
...
public final class Cmd {
  ...
  /**
   * Prints Jvpiter help message and exit.
   *
   * @param force if true force print
   */
  private static void help(boolean force) {
    if (force || Flags.HELP) {
      title(true, false);
      IO.stdout().println("usage: jvpiter [options] <files>");
      IO.stdout().println(Data.EOL + "[General Options]");
      IO.stdout().println("  -h, --help               show Jvpiter help message and exit");
      IO.stdout().println("  -v, --version            show Jvpiter version");
      IO.stdout().println("  -l, --license            show Jvpiter license");
      ...
      IO.stdout().println();
      IO.stdout().println("Please report issues at https://github.com/andrescv/Jvpiter/issues");
      // exit simulator
      if (Flags.HELP) {
        Jvpiter.exit(0);
      } else {
